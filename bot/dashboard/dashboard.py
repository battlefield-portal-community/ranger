...
try:
    if not os.getenv("DB_DEBUG", None):
        ...
        origins = [
            ...
            "https://stilllearning.tech",
            "http://stilllearning.tech",
        ]
        ...
        async def check_for_login(request: Request, call_next):
            ...
            if not any(map(lambda x: request.url.path.startswith(x), no_login)):
                if (
                    os.getenv("DEBUG_SERVER", "").lower() != "true"
                    and not request.url.path.startswith("/login")
                    and request.session.get("discord_user", None) is None
                ):
        ...
        async def finish_login(request: Request, code: str):
            ...
            if int(os.getenv("GUILD_ID")) in [guild.id for guild in guilds]:
                for guild in guilds:
                    if int(os.getenv("GUILD_ID")) == guild.id and (
                        guild.permissions & 0x0000000000000008
                    ):
        ...
        async def read_root(request: Request):
            if files := await get_files("global_config"):
                ...
                return templates.TemplateResponse(
                    "index.html",
                    {"request": request, "schema": schema, "start_val": config},
                )
        ...
        async def return_config(request: Request, config: str):
            if files := await get_files(config):
                with open(files[0]) as config_file, open(files[1]) as schema_file:
                    saved_config, schema = json.load(config_file), json.load(
                        schema_file
                    )

            return templates.TemplateResponse(
                "config_editor.html",
                {"request": request, "schema": schema, "start_val": saved_config},
            )
        ...
        async def return_file(
            config_name: str, config: bool = False, schema: bool = False
        ) -> dict:
            if config or schema:
        ...
        async def channel_name(id: int):
            ...
            if channel:
                return {"channel": {"id": id, "name": channel.name}}
            else:
                return {"channel": {}}
        ...
        async def get_files(config_name: str) -> tuple[Path, Path] | bool:
            schema_files = [
                ...
                for config_file in list(
                    (configs_base / "schemas" / "ranger").glob("*.schema.json")
                )
            ]
            if f"{config_name}.schema" in [file.stem for file in schema_files]:
                ...
                return (
                    config_file,
                    configs_base / "schemas" / "ranger" / f"{config_name}.schema.json",
                )
            else:
                return False
